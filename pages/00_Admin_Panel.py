...
Aktor: Admin
Smart Contract: RoleManager.assignRole(), removeRole(), getRole()
"""
...
<div class="page-header">
    ...
    <div style="margin-top: 12px; font-size: 0.75rem; color: #A78BFA;">
        📋 Smart Contract: <code style="background: rgba(124,58,237,0.1); padding: 2px 8px; border-radius: 4px;">RoleManager.assignRole() / removeRole()</code>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_assign:
    ...
    <div class="form-card" style="margin-top: 16px;">
        <div style="font-family: 'Space Grotesk', sans-serif; font-size: 1.1rem; font-weight: 600; 
             color: #F87171; margin-bottom: 20px;">🗑️ Hapus Peran Pengguna</div>
    """, unsafe_allow_html=True)
    
    with st.form("form_remove_role"):
        ...
        if submitted_remove:
            if not remove_address.strip():
                ...
                with st.spinner("⏳ Menghapus peran..."):
                    w3 = st.session_state.w3
                    contracts = st.session_state.contracts
                    role_manager = contracts['RoleManager']
                    
                    contract_func = role_manager.functions.removeRole(checksum)
                    result = build_transaction(
                        w3, contract_func,
                        st.session_state.wallet_address,
                        st.session_state.private_key
                    ...
                    if result['success']:
                        st.success(f"✅ Peran dari `{checksum[:20]}...` berhasil dihapus!")
                    else:
                        st.error(f"❌ Gagal: {result['error']}")
...
with col_info:
    ...
    <div class="form-card">
        <div style="font-family: 'Space Grotesk', sans-serif; font-size: 1rem; font-weight: 600; 
             color: #A78BFA; margin-bottom: 16px;">🔍 Cek Peran Pengguna</div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🔍 Cek Peran", key="btn_check_role"):
        if check_addr.strip() and st.session_state.get('ganache_connected'):
            ...
            try:
                ...
                contracts = st.session_state.contracts
                role = contracts['RoleManager'].functions.getRole(checksum).call()
                ...
                <div style="background: rgba(124,58,237,0.08); border: 1px solid rgba(167,139,250,0.2);
                     border-radius: 12px; padding: 16px;">
                    <div style="font-family: monospace; font-size: 0.75rem; color: #C4B5FD; margin-bottom: 8px;">
                        {checksum}
                    </div>
    ...
    <div class="form-card" style="margin-top: 16px;">
        <div style="font-family: 'Space Grotesk', sans-serif; font-size: 1rem; font-weight: 600; 
             color: #A78BFA; margin-bottom: 16px;">📋 Akun Simulasi (scenario2.json)</div>
    """, unsafe_allow_html=True)
    
    for role_name, addr in SIMULATION_ACCOUNTS.items():
        bg_color, text_color = get_role_color(role_name)
        
        # Cek peran aktual di blockchain jika terhubung
        actual_role = ""
        if st.session_state.get('ganache_connected'):
            try:
                from web3 import Web3
                checksum = Web3.to_checksum_address(addr)
                actual_role = st.session_state.contracts['RoleManager'].functions.getRole(checksum).call()
            except Exception:
                pass
        ...
        <div class="account-card">
            ...
            <div style="font-size: 0.7rem; color: #A78BFA;">
                Peran di BC: <strong>{actual_display}</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="background: rgba(124,58,237,0.05); border: 1px solid rgba(124,58,237,0.12);
        ...
        <div style="font-weight: 600; color: #A78BFA; margin-bottom: 8px;">ℹ️ Catatan</div>
        <ul style="margin: 0; padding-left: 16px; line-height: 1.8;">
            <li>Peran yang valid: Penangkar, Petani, Pengepul, Perusahaan</li>
            <li>Satu wallet hanya bisa memiliki satu peran</li>
            <li>Admin adalah pemilik kontrak (deployer)</li>
            <li>Perubahan peran tercatat sebagai event di blockchain</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
